...
def extract_resolution_from_horizontal_resolution(horizontal_resolution):
    """Extracts resolution number from horizontal_resolution field. Returns as string like '44' or '22'"""
...
def format_domain(domain, horizontal_resolution=None):
    """Return CORDEX domain code with resolution, e.g., 'AFR-44'"""
...
def make_dataset_id(domain, gcm_model_name, ensemble_member, rcm_model_name, experiment, horizontal_resolution=None):
    """Generate dataset_id in canonical format using underscores.

    Format: DOMAIN-RESOLUTION_GCM_EXPERIMENT_ENSEMBLE_RCM
    """
    domain_code = format_domain(domain, horizontal_resolution) if domain else ''
    gcm_name = sanitize_preserve_case(gcm_model_name) if gcm_model_name else ''
    rcm_name = sanitize_preserve_case(rcm_model_name) if rcm_model_name else ''
    experiment_clean = clean_token(experiment) if experiment else ''
    ensemble_clean = ensemble_member or ''  # keep ensemble string as-is
